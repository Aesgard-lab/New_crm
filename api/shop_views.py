...
from core.api_views import SafeAPIView

class ShopView(SafeAPIView):
    """
    ...
    def get(self, request):
        # Optimize Client query
        from memberships.services import get_cached_gym_plans
        from products.services import get_cached_gym_products
        from services.services import get_cached_gym_services
        
        client = get_object_or_404(
            Client.objects.select_related('gym').prefetch_related(
                'memberships',
                'memberships__plan',
                'payment_methods'
            ), 
        ...
        # Check if client has active membership (in memory using prefetch)
        has_active_membership = any(
            m.status in ['ACTIVE', 'PENDING'] for m in client.memberships.all()
        )
        
        # Check if client has payment method (in memory using prefetch)
        ...
        # Get active membership plans available for online purchase (CACHED)
        active_plans = get_cached_gym_plans(gym.id)
        ...
        # Get active products available for online purchase (CACHED)
        products_data = get_cached_gym_products(gym.id)
        ...
        # Get active services available for online booking (CACHED)
        active_services = get_cached_gym_services(gym.id)
...
class RequestInfoView(SafeAPIView):
    """
...
class ScheduleMembershipChangeView(SafeAPIView):
    """
